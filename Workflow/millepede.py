...
def run_convert(input_dir: Path, work_dir: Path, bin_dir: Path) -> None:
    ...
    """
    cmd = [
        str(bin_dir / '1convert'),
        '-i', str(input_dir),
        '-o', str(work_dir / 'mp2input'),
    ]
...
def generate_inputforalign(
    ...
    Steps mirror the old millepede.py post-processing chain:
      cp  ../1reco/inputforalign.txt  ./inputforalign_temp.txt
      5.1PedetoDB_ss  < millepede.res >> inputforalign_temp.txt
      5.2add_param    < inputforalign_temp.txt > inputforalign_new.txt
      cp  inputforalign_new.txt  ../inputforalign.txt
    ...
    """
    inputforalign_temp = work_dir / 'inputforalign_temp.txt'
    inputforalign_new  = work_dir / 'inputforalign_new.txt'
    millepede_res      = work_dir / 'millepede.res'

    # Start from the current inputforalign.txt
    shutil.copy2(inputforalign_in, inputforalign_temp)

    # Append pede DB entries: 5.1PedetoDB_ss <millepede.res >>inputforalign_temp.txt
    ...
    with open(millepede_res, 'r') as res_in, \
        ...
        subprocess.run(
            [str(bin_dir / '5.1PedetoDB_ss')],
            stdin=res_in, stdout=tmp_out,
            check=True, cwd=work_dir,
        )

    # Merge parameters: 5.2add_param <inputforalign_temp.txt >inputforalign_new.txt
    ...
    with open(inputforalign_temp, 'r') as tmp_in, \
        ...
        subprocess.run(
            [str(bin_dir / '5.2add_param')],
            stdin=tmp_in, stdout=new_out,
            check=True, cwd=work_dir,
        )

    # Copy final result to the iteration root for the next reco job
    shutil.copy2(inputforalign_new, inputforalign_out)
    print(f"Updated inputforalign written to: {inputforalign_out}")
...
def main() -> int:
    ...
    # ── Step 4: parse and run PedeStep chain ────────────────────────────
    try:
    ...
    try:
        ...
        for i, step in enumerate(steps):
            ...
            current = step.run(current, work_dir, steering)
            if i < n - 1:
